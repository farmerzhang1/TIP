  ...
  def visit(node: AstNode, arg: Unit): Unit = {
    ...
    node match {
      case program: AProgram =>
        for (f <- program.funs) visit(f, ()) // <--- Complete here
      ...
      case bin: ABinaryOp =>
        bin.operator match {
          ...
          case _ => unify(bin.left, IntType()); unify(bin.right, IntType()); unify(bin, IntType()); // <--- Complete here
        }
      case un: AUnaryOp =>
        un.operator match {
          case DerefOp => unify(PointerType(un), un.subexp) // <--- Complete here
        }
      case alloc: AAlloc =>
        unify(alloc, PointerType(alloc.exp)) // <--- Complete here
      case ref: AVarRef =>
        unify(ref, PointerType(ref.id)) // <--- Complete here
      case _: ANull => unify(node, PointerType(FreshVarType())) // <--- Complete here
      case fun: AFunDeclaration =>
        unify(fun, FunctionType(fun.params, fun.stmts.ret.exp)) // <--- Complete here
      case call: ACallFuncExpr =>
        unify(call.targetFun, FunctionType(call.args, call)) // <--- Complete here
      case _: AReturnStmt =>
      case rec: ARecord =>
